    ...
    public func doRender(scene: SCNScene, withOptions options: VideoRendererOptions, until: @escaping () -> (Bool), andThen: @escaping (_: URL) -> () ) {
        ...
        input.requestMediaDataWhenReady(on: self.frameQueue, using: {
            ...
            if until() {
                ...
                let snapshotTime = self.timerDaDaDoo
                let presentationTime = CMTimeMake(Int64(600 * snapshotTime), 600)
                
                
                //print(self.timerDaDaDoo)
                ...
                if(snapshotTime > self.previousTime || self.previousTime == 0){
                ...
                self.previousTime = snapshotTime
